// crawl-polarization.mjs
// polarization 축 크롤러: CBO(Congressional Budget Office) 공식 발간물 RSS만 수집 (allowlist = 1차 소스 한정)
// CBO는 소득분배(K자형 양극화)·재정 전망 보고서를 정기 발간하는 미 의회 산하 공식기관
...
// 1차 소스만 (CBO 공식). 추후 Reuters/Bloomberg 등 2차 소스 추가 시
// source_tier: "primary" | "secondary" 로 구분해서 신뢰도 축약 유지.
const SOURCES = [
  {
    name: "Congressional Budget Office - All Publications",
...
async function main() {
  ...
  for (const source of SOURCES) {
    try {
      ...
      console.error(`[crawl-polarization] source failed: ${source.name} — ${err.message}`);
    }
  }
  ...
  if (fresh.length === 0) {
    console.log("[crawl-polarization] no new items");
  ...
  console.log(`[crawl-polarization] ${fresh.length} new items -> ${outPath}`);
}
