// crawl-us_investment.mjs
// us_investment 축 크롤러: SEC(미 증권거래위원회) 공식 보도자료 RSS만 수집 (allowlist = 1차 소스 한정)
...
// 1차 소스만 (SEC 공식). 추후 Reuters/Bloomberg 등 2차 소스 추가 시
...
async function main() {
  ...
  for (const source of SOURCES) {
    try {
      ...
      console.error(`[crawl-us_investment] source failed: ${source.name} — ${err.message}`);
    }
  }

  if (fresh.length === 0) {
    console.log("[crawl-us_investment] no new items");
    return;
  }
  ...
  console.log(`[crawl-us_investment] ${fresh.length} new items -> ${outPath}`);
}
