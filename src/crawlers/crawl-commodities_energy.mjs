// crawl-commodities_energy.mjs
// commodities_energy 축 크롤러: EIA(미 에너지정보청) 공식 발행물 RSS만 수집 (allowlist = 1차 소스 한정)
...
// 1차 소스만 (EIA 공식). 추후 Reuters/Bloomberg 등 2차 소스 추가 시
...
async function main() {
  ...
  for (const source of SOURCES) {
    try {
      ...
      console.error(`[crawl-commodities_energy] source failed: ${source.name} — ${err.message}`);
    }
  }

  if (fresh.length === 0) {
    console.log("[crawl-commodities_energy] no new items");
    return;
  }
  ...
  console.log(`[crawl-commodities_energy] ${fresh.length} new items -> ${outPath}`);
}
