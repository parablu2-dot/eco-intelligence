// crawl-productivity_ai.mjs
// productivity_ai 축 크롤러: NIST(미 국립표준기술연구소) 공식 뉴스 RSS만 수집 (allowlist = 1차 소스 한정)
// NIST는 미국 AI 정책·표준(AI RMF 등)을 주관하는 공식기관
...
// 1차 소스만 (NIST 공식). 추후 Reuters/Bloomberg 등 2차 소스 추가 시
...
async function main() {
  ...
  if (fresh.length === 0) {
    console.log("[crawl-productivity_ai] no new items");
  ...
  console.log(`[crawl-productivity_ai] ${fresh.length} new items -> ${outPath}`);
}
