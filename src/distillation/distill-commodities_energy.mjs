...
async function main() {
  ...
  if (dateArg) {
    try {
      ...
      console.log(`[distill-commodities_energy] notes for ${today} already exist, skip (no overwrite)`);
      return;
    } catch {}
  }
  ...
  if (notes.length === 0) {
    if (failed > 0) recordLlmError("distill-commodities_energy", `all ${failed} item(s) failed — see log above`);
