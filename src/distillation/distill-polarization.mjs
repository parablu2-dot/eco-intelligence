...
async function distillOne(item, schema) {
  const userPrompt = `axis: polarization
title: ${item.title}
...
async function main() {
  ...
  if (dateArg) {
    try {
      ...
      console.log(`[distill-polarization] notes for ${today} already exist, skip (no overwrite)`);
      return;
    } catch {}
  }
  ...
  try {
    ...
    console.log(`[distill-polarization] no raw file for today: ${rawPath}`);
    return;
  }
  ...
  for (const item of rawItems) {
    try {
      ...
      failed++;
      console.error(`[distill-polarization] failed on ${item.url}: ${err.message}`);
    }
  }

  if (notes.length === 0) {
    if (failed > 0) recordLlmError("distill-polarization", `all ${failed} item(s) failed — see log above`);
    console.log("[distill-polarization] no notes produced");
    return;
  }

  const outPath = path.join(DAILY_DIR, `${AXIS}_${today}.json`);
  await fs.writeFile(outPath, JSON.stringify(notes, null, 2));
  console.log(`[distill-polarization] ${notes.length} notes -> ${outPath}`);
}
