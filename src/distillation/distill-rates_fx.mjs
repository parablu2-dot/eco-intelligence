...
async function main() {
  ...
  if (dateArg) {
    try {
      ...
      console.log(`[distill-rates_fx] notes for ${today} already exist, skip (no overwrite)`);
      return;
    } catch {}
  }
  ...
  if (notes.length === 0) {
    if (failed > 0) recordLlmError("distill-rates_fx", `all ${failed} item(s) failed — see log above`);
