// distill-productivity_ai.mjs
// crawl-productivity_ai.mjs가 만든 raw JSON을 읽어, 신규 원문만 Claude API로
...
const SYSTEM_PROMPT = `너는 거시경제 productivity_ai(생산성 혁명, AI) 축의 distillation 엔진이다.
입력된 NIST(미 국립표준기술연구소) 공식 뉴스 원문을 아래 JSON schema에 맞는 단일 객체로만 출력한다.
...
async function distillOne(item, schema) {
  const userPrompt = `axis: productivity_ai
title: ${item.title}
...
async function main() {
  ...
  if (dateArg) {
    try {
      ...
      console.log(`[distill-productivity_ai] notes for ${today} already exist, skip (no overwrite)`);
      return;
    } catch {}
  }
  ...
  try {
    ...
    console.log(`[distill-productivity_ai] no raw file for today: ${rawPath}`);
    return;
  }
  ...
  for (const item of rawItems) {
    try {
      ...
      failed++;
      console.error(`[distill-productivity_ai] failed on ${item.url}: ${err.message}`);
    }
  }

  if (notes.length === 0) {
    if (failed > 0) recordLlmError("distill-productivity_ai", `all ${failed} item(s) failed — see log above`);
    console.log("[distill-productivity_ai] no notes produced");
    return;
  }

  const outPath = path.join(DAILY_DIR, `${AXIS}_${today}.json`);
  await fs.writeFile(outPath, JSON.stringify(notes, null, 2));
  console.log(`[distill-productivity_ai] ${notes.length} notes -> ${outPath}`);
}
