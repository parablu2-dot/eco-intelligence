// distill-us_investment.mjs
// crawl-us_investment.mjs가 만든 raw JSON을 읽어, 신규 원문만 Claude API로
...
const AXIS = "us_investment";
const DAILY_DIR = path.resolve("data/daily");

const SYSTEM_PROMPT = `너는 거시경제 us_investment(미국 투자, 재정지배·달러패권) 축의 distillation 엔진이다.
입력된 SEC(미 증권거래위원회) 공식 보도자료 원문을 아래 JSON schema에 맞는 단일 객체로만 출력한다.
...
async function distillOne(item, schema) {
  const userPrompt = `axis: us_investment
title: ${item.title}
...
id는 "YYYYMMDD_us_investment_NN" 형식으로 임의 부여, date는 published 기준(YYYY-MM-DD), source_url은 url 값을 그대로 사용.
schema:
...
async function main() {
  ...
  try {
    ...
    console.log(`[distill-us_investment] no raw file for today: ${rawPath}`);
    return;
  }
  ...
  for (const item of rawItems) {
    try {
      ...
      console.error(`[distill-us_investment] failed on ${item.url}: ${err.message}`);
    }
  }

  if (notes.length === 0) {
    console.log("[distill-us_investment] no notes produced");
    return;
  }

  const outPath = path.join(DAILY_DIR, `${AXIS}_${today}.json`);
  await fs.writeFile(outPath, JSON.stringify(notes, null, 2));
  console.log(`[distill-us_investment] ${notes.length} notes -> ${outPath}`);
}
