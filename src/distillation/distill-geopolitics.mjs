// distill-geopolitics.mjs
// crawl-geopolitics.mjs가 만든 raw JSON을 읽어, 신규 원문만 Claude API로
...
const SYSTEM_PROMPT = `너는 거시경제 geopolitics(지정학적 리스크) 축의 distillation 엔진이다.
입력된 미 국무부 공식 발표 원문을 아래 JSON schema에 맞는 단일 객체로만 출력한다.
...
async function distillOne(item, schema) {
  const userPrompt = `axis: geopolitics
title: ${item.title}
...
async function main() {
  ...
  if (dateArg) {
    try {
      ...
      console.log(`[distill-geopolitics] notes for ${today} already exist, skip (no overwrite)`);
      return;
    } catch {}
  }
  ...
  try {
    ...
    console.log(`[distill-geopolitics] no raw file for today: ${rawPath}`);
    return;
  }
  ...
  for (const item of rawItems) {
    try {
      ...
      failed++;
      console.error(`[distill-geopolitics] failed on ${item.url}: ${err.message}`);
    }
  }

  if (notes.length === 0) {
    if (failed > 0) recordLlmError("distill-geopolitics", `all ${failed} item(s) failed — see log above`);
    console.log("[distill-geopolitics] no notes produced");
    return;
  }

  const outPath = path.join(DAILY_DIR, `${AXIS}_${today}.json`);
  await fs.writeFile(outPath, JSON.stringify(notes, null, 2));
  console.log(`[distill-geopolitics] ${notes.length} notes -> ${outPath}`);
}
