// distill-fed.mjs
// crawl-fed.mjs가 만든 raw JSON을 읽어, 신규 원문만 Claude API로
...
const SYSTEM_PROMPT = `너는 거시경제 fed_policy(연준/통화정책) 축의 distillation 엔진이다.
입력된 연준 공식 발표 원문을 아래 JSON schema에 맞는 단일 객체로만 출력한다.
...
async function distillOne(item, schema) {
  const userPrompt = `axis: fed_policy
title: ${item.title}
...
async function main() {
  ...
  if (dateArg) {
    try {
      ...
      console.log(`[distill-fed] notes for ${today} already exist, skip (no overwrite)`);
      return;
    } catch {}
  }
  ...
  try {
    ...
    console.log(`[distill-fed] no raw file for today: ${rawPath}`);
    return;
  }
  ...
  console.log(`[distill-fed] ${notes.length} notes -> ${outPath}`);
}
